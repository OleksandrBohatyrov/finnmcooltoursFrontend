...
function TourDetails({ token }) {
  ...
  const apiUrl = `http://62.60.157.133:5000/api/records?tourType=${encodeURIComponent(tourType)}`;
  ...
  const markCheckedIn = async (id) => {
    try {
      const res = await fetch(`http://62.60.157.133:5000/api/records/${id}/checkin`, {
        method: 'POST',
  ...
  const removeCheckedIn = async (id) => {
    try {
      const res = await fetch(`http://62.60.157.133:5000/api/records/${id}/remove-checkin`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (!res.ok) throw new Error(`HTTP error: ${res.status}`);
